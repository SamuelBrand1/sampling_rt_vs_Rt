...
Generation interval misspecification impacted all models (Figure @fig-scenario-performance, Panel B), with <process> showing <greatest/least> sensitivity to assumptions.
When generation intervals were misspecified as too short, performance degraded by <percentage>% on average, while too-long specifications resulted in <different impact> (SI Figure @fig-si-scenario-details, Panel C).
...
**Figure 2: Scenario-Stratified Performance (`fig-scenario-performance`)**
...
**Figure 3: Sierra Leone Case Study (`fig-case-study-performance`)**
...
- No exploration of uncertain delay or generation distributions
- Only explored a single delay distribution but the delay from infection to report could play an important role
...
- Time-varying delays, delay misspecification
