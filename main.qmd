...
For our Bayesian inference framework, we specify the following prior distributions for model parameters:

1. **Latent process parameters**:
   - For random walk: $\sigma \sim \text{Half-Normal}(0, 0.5)$
   - For AR(1): $\phi \sim \text{Beta}(2, 2)$ transformed to $(-1, 1)$, $\sigma \sim \text{Half-Normal}(0, 0.5)$
   - For differenced AR(1): $\psi \sim \text{Beta}(2, 2)$ transformed to $(-1, 1)$, $\sigma \sim \text{Half-Normal}(0, 0.5)$

2. **Initial condition parameters**:
   - $\log(I_0) \sim \text{Normal}(0, 2)$ for all models
   - For renewal process: $\log(R_1) \sim \text{Normal}(0, 0.5)$

3. **Observation model parameters**:
   - Ascertainment fractions: $\text{logit}(\rho_1), \text{logit}(\rho_2) \sim \text{Normal}(0, 1)$
   - Delay distribution parameters: $\theta_1, \theta_2 \sim \text{Normal}(0, 1)$ with appropriate transformations
   - Overdispersion: $\log(\alpha) \sim \text{Normal}(0, 1)$

These prior choices reflect our general understanding of epidemic dynamics while remaining weakly informative to allow the data to drive inference. The priors for latent process parameters are chosen to ensure stationarity of the underlying processes, while observation model priors reflect typical ranges observed in infectious disease surveillance systems.
...
1. Take a fixed time series of Rt for 160 days. 
2. Add noise to the fixed Rt estimates draws from a N(0, 0.1) with a fixed seed of `12345`.
3. Simulate daily incidence starting from $I_0 = 100$ cases and a fixed generation interval (GI) probability mass function (PMF) specific to the given scenario.
4. The delay between infection and case ascertainment is represented as a convolution of the true incidence time series with a Gamma distribution. Based on the code review, we use a Gamma(4, 5/4) distribution for the delay, which has a mean of 5 days and standard deviation of 2.5 days. This is implemented using the `LatentDelay` modifier in the EpiAware framework.
5. Simulate day of the week periodicity using the `ascertainment_dayofweek` function, which applies a day-of-week effect through a logit-scale relative ascertainment model. The default configuration uses a 7-element vector `logit_daily_ascertainment` set to [0, 0, 0, 0, 0, -0.5, -0.5], which after softmax transformation gives differential reporting by day of week.
6. Simulate additional negative binomial observation noise on the delayed cases with the mean of the true cases and overdispersion determined by a cluster factor of 0.05. This is implemented using the `NegativeBinomialError` model in the EpiAware framework.
...
We use three generation intervals (GIs), corresponding to pathogens with short, medium, and long GIs for each infectious disease scenario. The GIs are implemented as Gamma distributions with a fixed standard deviation of 2.0 days and varying means. These continuous distributions are discretized using double interval censoring (assuming a uniform distribution over each daily time interval) as implemented in the `censored_pmf` function in the EpiAware framework, with the default truncation at the 99th percentile of the distribution.
...
These distributions are specified by their mean and standard deviation in the code, with the shape parameter calculated as (mean/std)² and the scale parameter calculated as std²/mean. The discretized probability mass functions are normalized to sum to 1.
